            ...
            VStack(alignment: .leading) {
                ...
                VStack(alignment: .leading,spacing: 20) {
                    ...
                    VStack(alignment: .trailing, spacing: 0) {
                        if let banks = viewModel.bootConfigResponse?.data?.allowPayMode?.mmoney?.bank, !banks.isEmpty {
                            ScrollView(.horizontal, showsIndicators: false) {
                                HStack(spacing: 20) {
                                    ForEach(banks.indices, id: \.self) { index in
                                        if let bankItem = banks[safe: index] {
                                            BankView(bankItem: bankItem)
                                        }
                                    }
                                }
                                .padding(.leading, 20)
...
struct BankView: View {
    let bankItem: MMoney.Bank

    var body: some View {
        VStack {
            VStack(alignment: .center, spacing: 0) {
                AsyncImage(url: URL(string: bankItem.dispImg ?? "")) { image in
                    image
                        .resizable()
            ...
            VStack(alignment: .leading, spacing: 0) {
                Text(bankItem.dispnm ?? "Unknown")
                    .font(Font.custom("Open Sans", size: 12).weight(.semibold))
                    .foregroundColor(Color(red: 0, green: 0.12, blue: 0.18))
                    .lineLimit(2)
